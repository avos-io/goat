...
func TestContext(t *testing.T) {
	...
	stream, err := newServerStream(ctx, 0, "", rw.Read, rw.Write)
	is.NoError(err)
...
func TestHeaders(t *testing.T) {
	t.Run("SendHeader", func(t *testing.T) {
		...
		method := "my_method"
		rw := mocks.NewRpcReadWriter(t)
		stream, err := newServerStream(ctx, streamId, method, rw.Read, rw.Write)
		is.NoError(err)
	...
	t.Run("SendHeader write error", func(t *testing.T) {
		...
		rw := mocks.NewRpcReadWriter(t)
		stream, err := newServerStream(context.Background(), 0, "", rw.Read, rw.Write)
		is.NoError(err)
